...
export default function RoundRulesPage() {
  const { roundId } = useParams();
  const [, setLocation] = useLocation();
  const { toast } = useToast();

  const { data: round, isLoading: roundLoading } = useQuery<Round>({
    queryKey: ['/api/rounds', roundId],
    enabled: !!roundId,
  });

  const { data: rules, isLoading: rulesLoading } = useQuery<RoundRules>({
    queryKey: ['/api/rounds', roundId, 'rules'],
    enabled: !!roundId,
  });
      ...
      <div className="p-8">
        <div className="mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900" data-testid="heading-rules">Round Proctoring Rules</h1>
              <p className="text-gray-600 mt-1">{round?.name}</p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-3xl">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-800 mt-1">
                    These rules enforce test integrity by controlling participant behavior during this round.
                    Changes will apply to all future test attempts for this round.
                  </p>
                </div>
              </div>
            </CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Set up automated monitoring and violation handling for this round
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
